...
- **⑦ 跳过代价已量化（事后补记）**：⑦ 跳过代价已量化——PR #5 先合并、⑦ 被跳，事后补 ⑦ 逮出 F-1/F-2 两个重要 bug（已漏进 main），门禁 ⑥（gates + CODEOWNERS）与自评勾的"测试未弱化"复选框均未拦住 → 独立评审步不可由机器关替代。证据：[`servers/generation-service/review/2026-06-10-review-gen-a1.md`](../../../servers/generation-service/review/2026-06-10-review-gen-a1.md)（事后 ⑦ 报告 + 修复轮处理状态）。
- **回流候选（设计层·门禁应强制 ⑦ 的形式）**：②层 path-diff 门禁应含一项——PR 改动命中高影响区（`app/**`）时必须随附对应 `review/*.md`，否则 gates 红。此项若早就位，PR #5（无 review 即合）当场被挡 → 逼 ⑦ 在合并前发生。但仅能强制"形式"：机器可验"有 review 文件 / 有非作者 approve"，验不了评审的实质深度与独立性（空壳 review、秒点 approve 均满足形式），受 §12 单脑天花板限——作用是把"静默跳过"变成"须主动造假"（抬地板），非根治。同簇 [[B10]]（CI 自指可掏空）/[[B11]]（软门禁靠人记得）/本体（③ 可自放行）/CODEOWNERS approve 可浅——统一底层：独立"判断"步是软肋，机器只验动作发生、验不了判断真发生。
